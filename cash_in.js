...
let connector = "in"; // Use a different variable name instead of "in"
let new_date = date + ' ' + connector + ' ' + REFERANCE; 
console.log(new_date);// Concatenate the date and connector
...
function displayExpenses(date) {
    ...
    let totalAmount_in = 0; // Variable to store the total amount
    ...
    onValue(expensesRef, (snapshot) => {
        snapshot.forEach((childSnapshot) => {
            ...
            if (expense.date == date) {
                ...
                if (!itemsDisplayed.has(expense.item)) {
                    ...
                    totalAmount_in += parseFloat(expense.amount); // Add amount to total
                }
            }
        }
        });
        ...
        totalAmountInput.value = totalAmount_in;
        
        localStorage.setItem(new_date,totalAmount_in );
    });
}
